...
function simulate_LIF_neuron_fixed_F(A, d_ie, f_ie, tau_d, tau_f, dt, T, S_input)
    τ_m = 10.0       
    V_thresh = -50.0  
    V_reset = -75.0   
    V_rest = -75.0    
...
# Run simulation
time, Vs, spike_times, Hs = simulate_LIF_neuron(A, d_ee, f_ee, tau_d, tau_f, dt, T, S_input)
...
for tf in temporal_frequencies
    global S_input, time, Vs, spike_times, Hs, depression_ratio
    S_input = generate_spike_train(T, dt, first_spike_time, tf)
    time, Vs, spike_times, Hs = simulate_LIF_neuron_fixed_F(A, d_ie, f_ie, tau_d, tau_f, dt, T, S_input)
    if length(Hs) >= 2
        depression_ratio = Hs[2] / Hs[1]
...
# Plot results
using Plots
plot(temporal_frequencies, depression_ratios, xlabel="Temporal Frequency (Hz)", title = "d_ee=$d_ee f_ee=$f_ee d_ie=$d_ie f_ie=$f_ie", ylabel="Depression Ratio", label="E->E", marker=:circle, markercolor=:red,linewidth=2, linecolor=:red)
plot!(temporal_frequencies, depression_ratios_fixed_F, label="E->I", marker=:circle, linewidth=2, linecolor=:deepskyblue2,markercolor=:deepskyblue2)
